 ...
 * @since   2.2
 */
public strictfp class NameValidator extends Validator {
    /**
    ...
    private void validate(final GenericName object, final List<? extends LocalName> parsedNames) {
        ...
        if (parsedNames != null) {
            ...
            assertFalse(parsedNames.isEmpty(), "GenericName: getParsedNames() shall not return an empty list.");
    ...
    public void validate(final ScopedName object) {
        ...
        if (tail != null) {
            assertEquals(depth-1, tail.depth(), "ScopedName: tail() shall have one less element than the enclosing scoped name.");
            assertEquals(object.tip(), tail.tip(), "ScopedName: tip().toString() and tail.tip().toString() shall be equal.");
            if (parsedNames != null) {
