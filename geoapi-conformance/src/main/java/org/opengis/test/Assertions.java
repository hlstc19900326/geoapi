...
/**
 * Extension to JUnit assertion methods. This class adds some assertion methods in
 * addition of the ones provides by JUnit 5 {@link org.junit.jupiter.api.Assertions}.
 * Both JUnit and GeoAPI assertions can be used together with static import statements
 * like below:
 *
 * <pre>import static org.junit.jupiter.api.Assertions.*;
 * import static org.geoapi.test.Assertions.*;</pre>
 *
...
public final strictfp class Assertions {
    ...
    private static String nonNull(String message) {
        if (message != null) {
            ...
            if (n != 0) {
                return message.concat(Character.isLetterOrDigit(message.codePointBefore(n)) ? ": " : " ");
            }
        }
        return "";
    }
     ...
     * This method returns the given suffix if the message is null or empty.
     *
     * <p>Invoking this method is equivalent to invoking {@code nonNull(message).concat(suffix)}
     * except that the string is created only if needed.</p>
     *
     * @param  message  the message, or {@code null}.
     * @param  suffix   the suffix to append after the message.
     * @return the concatenated string, constructed only when requested.
     */
    private static Supplier<String> concat(final String message, final String suffix) {
        return () -> nonNull(message).concat(suffix);
    }
     ...
     */
    public static void assertInstanceOf(final Class<?> expectedType, final Object value, final String message) {
