...
public strictfp abstract class TransformTestCase extends TestCase {
     ...
     *
     * <blockquote><code>
     * return tolerance * StrictMath.abs(ordinate);
     * </code></blockquote>
     *
            ...
            throws TransformException
    {
        ...
        assertEquals(numPts, coordinates.length / sourceDimension,
    ...
    protected void verifyInverse(final double... coordinates) throws TransformException {
         ...
         */
        assertNotNull(coordinates,
                "Coordinates array expected in argument.");
        assertEquals(0, coordinates.length % sourceDimension,
                "Source dimension is not a divisor of the coordinates array length.");
        ...
        for (int i=0; i<numPts; i++) {
            ...
            assertNotNull(target,
                    "MathTransform.transform(DirectPosition, …) shall not return null.");
    ...
    protected float[] verifyConsistency(final float... sourceFloats) throws TransformException {
        ...
        if (true) { // MathTransform.transform(DirectPosition) is not optional in this test.
            ...
            for (int i=0; i < sourceDoubles.length; i += sourceDimension) {
                ...
                assertEquals(targetDimension, targetPosition.getDimension(),
                        "MathTransform.transform(DirectPosition) must return a position having " +
        ...
        if (transform instanceof MathTransform2D) {
            ...
            for (int i=0; i<sourceFloats.length;) {
                ...
                assertNotNull(target,
                        "MathTransform2D.transform(Point2D, …) shall not return null.");
    ...
    protected void verifyDerivative(final double... coordinate) throws TransformException {
        ...
        assertNotNull(derivativeDeltas,
                "TransformTestCase.derivativeDeltas shall be assigned a value.");
        assertFalse(derivativeDeltas.length == 0,
                "TransformTestCase.derivativeDeltas shall not be empty.");
        assertEquals (transform.getSourceDimensions(), coordinate.length,
